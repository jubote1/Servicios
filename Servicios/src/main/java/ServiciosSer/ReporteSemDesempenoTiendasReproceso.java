...
public class ReporteSemDesempenoTiendasReproceso {
	        ...
	        
	{
		...
		String respuesta = "";
		//Ahora realizamos el procesamiento de lo que es como tal el coraz�n del reporte
		//Vamos a incluir la l�gica para traer todas las tiendas y revisar las estad�sticas de los domiciliarios en dicho d�a y
		// de la cocina
		...
		for(int j = 0; j < tiendas.size(); j++)
		{
			...
			if(!tiendaTemp.getHosbd().equals(new String("")))
			{
				...
				respuesta = respuesta + "<table WIDTH='250' border='2'> <TH COLSPAN='6'> DESEMPENO DOMICILIARIOS " + tiendaTemp.getNombreTienda()  + "</TH> </tr>";
		...
		//Generamos el desempe�o de las cocinas de las tiendas
		for(int j = 0; j < tiendas.size(); j++)
		{
			tiendaTemp = tiendas.get(j);
			if(!tiendaTemp.getHosbd().equals(new String("")))
			{
				//Realizamos la labor con cada una de las tiendas
				respuesta = respuesta + "<table WIDTH='250' border='2'> <TH COLSPAN='6'> DESEMPE�O COCINA " + tiendaTemp.getNombreTienda()  + "</TH> </tr>";
				respuesta = respuesta + "<tr>"
			...
			correo.setAsunto("REPORTE DESEMPE�O TIENDAS entre fecha " + fechaAnterior + " y " + fechaActual);
