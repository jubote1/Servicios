...
public class ReporteSemanalDIDIReproceso {
	
	
	public void generarReporteDIDI()
	{
		...
		for(int i = 0; i < razonesSociales.size(); i++)
		{
			...
			for(Tienda tiendaTemp : tiendas)
			{
				...
				pedidosDomCOMTienda = PedidoDAO.obtenerPedidosPlataformasTiendaDetallada(razTemp.getIdRazon(), fechaAnterior, fechaActual,1,tiendaTemp.getIdTienda());
			...
			correo.setAsunto("Reporte Facturaci�n Semanal DIDI de la Raz�n Social " + razTemp.getNombreRazon() + " " + razTemp.getIdentificacion());
			correo.setContrasena(infoCorreo.getClaveCorreo());
			ArrayList correos = GeneralDAO.obtenerCorreosParametro("REPORTEDOMICILIOSDIDI");
			correo.setUsuarioCorreo(infoCorreo.getCuentaCorreo());
			correo.setMensaje("A continuaci�n el reporte semanal de pedidos tomados para DIDI separados por razones sociales entre las fechas " + fechaAnterior + " - " + fechaActual +  ": \n" + respuesta);
...
{
	ReporteSemanalDIDIReproceso reporteDomicios = new ReporteSemanalDIDIReproceso();
	reporteDomicios.generarReporteDIDI();
