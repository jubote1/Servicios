...
public class ServicioInventarios {
	...
	//M�todo para obtener la homologaci�n de los insumo con las diferentes tiendas
	public static int obtenerIdInsumoIntero(int idtienda, int idinsumotienda, String ipBodega)
	{
		int idInsumoInterno=0;
		ConexionBaseDatos con = new ConexionBaseDatos();
		Connection con1 = con.obtenerConexionBDInventarioURL(ipBodega);
		try
		{
	...
	{
		//Vamos a recuperar la IP de la Bodega para no quemar IPs
		String ipBodega = capaDAOPOS.ParametrosDAO.retornarValorAlfanumerico("URLCONTACTMONITOREO", false);
		...
		String nombreTienda = TiendaDAO.obtenerNombreTienda();
			...
			Connection conInventario = conexion.obtenerConexionBDInventarioURL(ipBodega);
			Statement stmInventario;
			ArrayList<InsumoAlerta> insumosAlertas = InsumoAlertaDAO.retornarInsumosAlerta(ipBodega);
			...
			try
			{
				...
				while(rsTiendaPixel.next())
				{
					...
					//Control para solo insertar los productos que tengan homologaci�n
					if(idinsumointerno > 0)
					{
						...
						for(int  i = 0; i < insumosAlertas.size(); i++)
						{
							...
							if(insTemp.getIdInsumo() == idinsumointerno)
							{
								...
								{
									// Se Acumula el insumo en la conformaci�n de la tabla
									...
									//Se prender� el indicador para env�o de correo y se adicionar� en la tabla de reportados
									if(!insumoReportado)
									{
				...
				// Al pasar este punto y no se ha salido es porque no se ha disparado excepci�n por lo tanto aqui podemos realizar el borrado de la tabla oficial y
				// y pasar la informaci�n del  temporal
				...
				//En este punto haremos la validaci�n de si hay que enviar el correo
				if(enviarCorreo)
				{
						...
						correo.setMensaje("A continuaci�n el reporte de insumos de inventario que est�n por debajo de los par�metros establecidos: \n" + cuerpoCorreo);
			...
			{
				System.out.println("Error en la conexi�n al Sistema POS tienda " + e.toString());
				try
				{
					conInventario.close();
				}catch(Exception ex)
			...
			//Pero no uses System.gc() en producción, ya que puede afectar el rendimiento.
			//System.gc();
	}
