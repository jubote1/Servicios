...
public class ReporteBloqueosTiendaReproceso {
		...
		for(Tienda tiendaTemp: tiendas)
		{
			...
			for(LogBloqueoTienda logTemp: bloqueosTienda)
			{
				...
				}else if(logTemp.getAccion().equals(new String("DESBLOQUEO")))
				{
					...
					try
					{
						dif = fechaDesbloqueo.getTime() - fechaBloqueo.getTime();
					}catch(Exception e)
					{
						dif = 0;
					}
					...
					{
						System.out.println("POSIBLE ERROR - fecha Inicial " + fechaInicial + " fecha bloqueo " + fechaBloqueo + " fechaDesbloqueo " + fechaDesbloqueo + " motivo " + motivo + " observacion " + observacion + " minutos " + difMinutos);
					}
					fechaBloqueo = null;
