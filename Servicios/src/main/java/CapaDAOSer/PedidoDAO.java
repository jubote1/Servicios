...
public class PedidoDAO {
	...
	/**
	 * M�todo que se encargar� de consultar los pedidos pendientes dada una fecha determinada y con origen de la tienda virtual, con el fin de alertar posteriormente en correo electr�nico
	 * @param fechaPed
	 * @return
	 */
	...
	/**
	 * M�todo que se encargar� de traer los posibles pedidos duplicados que se alertar�n de manera temprana con el fin de 
	 * @param fechaPed
	 * @return
	 */
	...
	//M�todo que retona totales por especialidades de pizzas en un d�a determinado
		...
		public static double obtenerTotalesPedidosSemanaFE(String fechaAnterior, String fechaPosterior , String url)
		{
			ConexionBaseDatos con = new ConexionBaseDatos();
			Connection con1 = con.obtenerConexionBDTiendaRemota(url);
			double totalVenta = 0;
			...
			{
				Statement stm = con1.createStatement();
				String consulta = "select SUM(d.total_neto) - (select IFNULL(SUM(b.valortotal),0) AS  total_neto FROM pedido a, detalle_pedido b, producto c WHERE a.idpedidotienda = b.idpedidotienda AND b.idproducto = c.idproducto AND a.fechapedido >= '"+ fechaAnterior +"' AND a.fechapedido <=  '" + fechaPosterior + "'  AND a.idmotivoanulacion IS NULL AND c.no_venta = 1) AS total_neto FROM pedido d WHERE d.fechapedido >= '" + fechaAnterior + "' AND d.fechapedido <=  '" + fechaPosterior +"'  AND d.idmotivoanulacion IS NULL and d.factura_generada = 1"; 
				ResultSet rs = stm.executeQuery(consulta);
				while(rs.next()){
					totalVenta = rs.getDouble(1);
					break;
		...
		//Comenzamos a generar una consulta por cada promoci�n
		
		/**
		 * M�todo que nos trae la cantidad de promociones vendida para una fecha en espec�fica con base en la fecha y el 
		 * String de conexi�n de la tienda
		 * @param fecha
		 * @param url
		 * @return
		 */
		...
		public static double obtenerTotalesCodigoFlash(String fecha , String url)
		{
			...
			try
			{
				...
				String consulta = "SELECT \r\n" + 
						...
						"AND pe.fechapedido = tabla.dia\r\n" + 
						"AND dp.idproducto IN(SELECT pr.idproducto FROM producto pr WHERE\r\n" + 
						"pr.descripcion like '%GD c�digo flash%')) AS MdDobleOnline\r\n" + 
						"FROM\r\n" + 
						"(\r\n" + 
						"SELECT pe.fechapedido as dia\r\n" + 
						"FROM pedido pe\r\n" + 
		...
		/**
		 * M�todo gen�rico que nos entrega la informaci�n por promoci�n, origen del canal y fecha.
		 * @param fecha
		 * @param idExcepcion
		 * @param origen
		 * @return
		...
		/**
		 * M�todo que retorna la cantidad de productos vendido por canal para un determinado producto
		 * @param fecha
		 * @param idProducto
		 * @param origen
		 * @param idTienda
		...
		/**
		 * M�todo que no retornar� la cantidad de c�digos promocionales redimidos en la fecha determinada
		 * @param fecha
		 * @return
		 */
		...
		/**
		 * M�todo que no retornar� la cantidad de pedidos domicilios.com en la fecha determinada
		 * @param fecha
		 * @return
		 */
		...
		/**
		 * M�todo que se encargar� de retornar los clientes a los que les aplicar�a el c�digo promocional para su env�o seg�n
		 * los par�metros enviados para su ejecuci�n.
		 * @param idTienda
		 * @param fechaInicial
		 * @param fechaFinal
		 * @param cantidad
		...
		/**
		 * M�todo que retorna los domicilios que llevo un domiciliario determinado en un rango de fechas
		 * @param fechaInicial
		 * @param fechaFinal
		...
		public static boolean actualizarPedidosPayu(String fechaInicial)
		{
			...
			try
			{
				...
				System.out.println("Actualizaci�n de pagos con PAYU");
				try
				{
					con1.close();
				}catch(Exception e1)
		...
		/**
		 * M�todo que retorna la cantidad de pedidos cumplidos dentro del rango de fechas usando log�stica.
		 * @param fechaAnterior
		...
		/**
		 * M�todo que retorna la cantidad de pedidos no cumplidos dentro del rango de fechas usando log�stica.
		 * @param fechaAnterior
		 * @param fechaPosterior
		 * @param url
		 * @return
		...
		/**
		 * M�todo que retorna el valor del descuento asumido por las plataformas.
		 * @param fechaInicial
		 * @param fechaFinal
		 * @param idTienda
		 * @return
		...
		public static ArrayList<PedidoFactElectronica> obtenerPedidosValidarFactElectronica( String fechaAnterior, String fechaPosterior, String hostBD) {
			 
			Logger logger = Logger.getLogger("log_file");
			ConexionBaseDatos con = new ConexionBaseDatos();
			Connection con2 = con.obtenerConexionBDTiendaRemota(hostBD);
			ArrayList<PedidoFactElectronica> pedidos = new ArrayList();
			boolean esPlataforma = false;
			try {
				 Statement stm = con2.createStatement();
				 String consulta = "SELECT a.idpedidotienda, b.cuerpo_solicitud, b.idlog  FROM pedido a, log_facturacion_electronica b WHERE a.idpedidotienda = b.idpedidotienda and a.fechapedido >= '" + fechaAnterior + "' AND a.fechapedido <= '" + fechaPosterior + "' AND !(a.estacion LIKE '%DIDI%' OR a.estacion LIKE '%RAPPI%') AND (a.factura_generada IS NULL OR a.factura_generada = 0)";
				 ResultSet rs = stm.executeQuery(consulta);
				 int idPedido = 0;
				 String cuerpoJSON;
				 int idLog = 0;
				 while(rs.next()) {
					...
					cuerpoJSON = rs.getString(2);
					idLog = rs.getInt(3);
					try
					{
						Gson gson = new Gson();
		                Type type = new TypeToken<Map<String, Object>>() {}.getType();
		                Map<String, Object> map = gson.fromJson(cuerpoJSON, type);
		                String documentNumber = (String) map.get("document_number");
		                String prefix = (String) map.get("prefix");
		                pedidos.add(new PedidoFactElectronica(idPedido, prefix, documentNumber, idLog));
					}catch(Exception e)
					{
						System.out.println(e.toString());
					}
		...
		public static boolean ActualizarPedidoFacturado( int idPedido, boolean factura_generada, String hostBD) {
			 Logger logger = Logger.getLogger("log_file");
			 ConexionBaseDatos con = new ConexionBaseDatos();
			 Connection con2 = con.obtenerConexionBDTiendaRemota(hostBD);
			try {
				 Statement stm = con2.createStatement();
				 String update = "update pedido set factura_generada =" + factura_generada + " where idpedidotienda= " + idPedido;
				stm.executeUpdate(update);
				stm.close();
				con2.close();
			} catch ( Exception e) {
		...
		/**
		 * Método que retorna los pedidos que están pendientes de generación de factura electrónica
		 * @param fechaAnterior
		 * @param fechaPosterior
		 * @param hostBD
		 * @return
		 */
		public static int obtenerPedidosPendientesFacturar(String hostBD) {
			...
			try {
				 Statement stm = con2.createStatement();
				 String consulta = "SELECT COUNT(*) FROM cola_factura_electronica";
				 ResultSet rs = stm.executeQuery(consulta);
				 while(rs.next()) {
					try
					{
		...
		/**
		 * Método que retornará los pedidos a los cuales hay que hacer seguimiento para verificar si están en el plan de fidelización
		 * @param fecha
		 * @param url
		 * @return
		 */
		public static ArrayList<PedidoPlanFidelizacion> obtenerPedidosValidarFidelizacion(String fecha , String url)
		{
			ConexionBaseDatos con = new ConexionBaseDatos();
			Connection con1 = con.obtenerConexionBDTiendaRemota(url);
			ArrayList<PedidoPlanFidelizacion> pedidos = new ArrayList();
			PedidoPlanFidelizacion pedidoTemp = new PedidoPlanFidelizacion();
			try
			{
				Statement stm = con1.createStatement();
