...
public class PedidoFueraTiempoDAO {
	...
	public static void insertarPedidoFueraTiempo(PedidoFueraTiempo pedFueraTiempo)
	{
		...
		String insert = "insert into pedido_fuera_tiempo (idpedido, idtienda, transact, tiempo_dado, tiempo_actual, por_desviacion, domiciliario, estado_pedido) values ( " + pedFueraTiempo.getIdPedido() + " , " + pedFueraTiempo.getIdTienda() + " , " + pedFueraTiempo.getTransact() + " , " + pedFueraTiempo.getTiempoDado() + " , " + pedFueraTiempo.getTiempoActual()  + " , " + pedFueraTiempo.getPorcDesviacion() + " , '" + pedFueraTiempo.getDomiciliario() + "' , '" + pedFueraTiempo.getEstadoPedido() + "')";
		try
		{
	...
	public static void ActualizarPedidoFueraTiempo(int idPedido, double porDesviacion, String domiciliario, String estadoPedido, double tiempoActualPedido)
	{
		ConexionBaseDatos conexion = new ConexionBaseDatos();
		Connection conContact = conexion.obtenerConexionBDContact();
		String update = "update pedido_fuera_tiempo set por_desviacion ="+  porDesviacion + ", domiciliario ='" + domiciliario + "' , estado_pedido = '" + estadoPedido + "', tiempo_actual =" + tiempoActualPedido+ "  where idpedido = " + idPedido;
		try
		{
			Statement stm= conContact .createStatement();
	...
	public static ArrayList<PedidoFueraTiempo> obtenerPedidoFueraTiempo(int idTienda, int numMinutos)
	{
		...
		try
		{
			...
			while(rs.next())
			{
				...
				pedFueraTemp = new PedidoFueraTiempo(idPedido, idTienda, transact, tiempoDado, tiempoActual, porcDesviacion, domiciliario, estadoPedido);
